 ...
 */
public final class Yin2 implements PitchDetector {
    /**
     ...
     */
    public Yin2(final float audioSampleRate, final int bufferSize) {
        this(audioSampleRate, bufferSize, DEFAULT_THRESHOLD);
    }

    public static Yin2 getInstance(float sampleRate,final int bufferSize) {
        return new Yin2(sampleRate,bufferSize);
    }
    /**
     * Create a new pitch detector for a stream with the defined sample rate.
     * Processes the audio in blocks of the defined size.
     ...
     */
    public Yin2(final float audioSampleRate, final int bufferSize, final double yinThreshold) {
    ...
    private void difference() {
        ...
        for (tau = 1; tau < yinBuffer.length; tau++) {
            for (index = 0; index < yinBuffer.length; index++) {
                delta = inputBuffer[index] - inputBuffer[index + tau];
